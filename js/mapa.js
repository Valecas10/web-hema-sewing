...
function inicializarMapa() {
    ...
    mapa = L.map('mapa-libre').setView(
        [
            estadoUbicacion.coords.lat,
            estadoUbicacion.coords.lon
        ],
        4
    );
        ...
        async () => {
            ...
            try {
                ...
                if (
                    ...
                    const direccion =
                        `${p.street || p.name || 'Direccion Desconocida'} ${p.housenumber || ''}`;
    ...
    selectProvincia.addEventListener(
        ...
        (e) => {
            ...
            if (
                ...
                const zoom =
                    zoomPorProvincia[prov];


                estadoUbicacion.provincia =
                    prov;

                estadoUbicacion.coords = {
                    lat,
                    lon
                };
                ...
                if (prov === 'CABA') {
                    ...
                    estadoUbicacion.ciudad =
                        'CABA';

                    inputCiudad.placeholder =
                        'Aplica para todo CABA';
        ...
        debounce(
            async () => {
                ...
                try {
                    ...
                    const url =
                        `https://photon.komoot.io/api/?q=${encodeURIComponent(query + ' ' + prov + ' Argentina')}&limit=10&bbox=${BBOX_ARG}`;
        ...
        debounce(
            async () => {
                ...
                try {
                    ...
                    const queryBusqueda =
                        `${calle}, ${ciudad}, ${provincia}, Argentina`;

                    const url =
                        `https://photon.komoot.io/api/?q=${encodeURIComponent(queryBusqueda)}&lat=${lat}&lon=${lon}&limit=10`;
...
function renderizarSugerenciasCiudad(
    features
) {
...
function renderizarSugerenciasCalle(
    features
) {
    ...
    features.forEach(lugar => {
        ...
        div.onclick = () => {
            ...
            direccionValidada =
                `${dirCompleta}, ${p.city || ciudadActual}`;
